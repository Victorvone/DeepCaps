...
decoder.add(
    layers.Deconvolution2D(
        256, 3, 3, subsample=(1, 1), border_mode="same", activation="relu"
    )
)
decoder.add(
    layers.Deconvolution2D(
        256, 3, 3, subsample=(2, 2), border_mode="same", activation="relu"
    )
)
decoder.add(
    layers.Deconvolution2D(
...
decoder.add(
    layers.Deconvolution2D(
        16, 3, 3, subsample=(1, 1), border_mode="same", activation="relu"
    )
)
decoder.add(
    layers.Deconvolution2D(
        8, 3, 3, subsample=(1, 1), border_mode="same", activation="relu"
    )
)
decoder.add(
    layers.Deconvolution2D(
        3, 3, 3, subsample=(1, 1), border_mode="same", activation="relu"
    )
)
...
lr_decay = callbacks.LearningRateScheduler(
    schedule=lambda epoch: args.lr * 0.5 ** (epoch // 5)
)
...
parallel_model.compile(
    optimizer=optimizers.Adam(lr=args.lr), loss=["mse"], metrics={"decoder": "loss"}
)

parallel_model.fit(
    vect, x256_train, batch_size=32, epochs=args.epochs, callbacks=[lr_decay]
)
