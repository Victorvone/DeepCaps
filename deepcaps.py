...
def DeepCapsNet(input_shape, n_class, routings):
    ...
    l = Conv2D(128, (3, 3), strides=(1, 1), activation="relu", padding="same")(
        l
    )  # common conv layer
...
def BaseCapsNet(input_shape, n_class, routings):
    ...
    l = Conv2DCaps(
        16, 6, kernel_size=(3, 3), strides=(2, 2), r_num=1, b_alphas=[1, 1, 1]
    )(l)

    l = FlattenCaps()(l)
    digits_caps = CapsuleLayer(
        num_capsule=10, dim_capsule=8, routings=routings, channels=0, name="digit_caps"
    )(l)
    l = CapsToScalars(name="capsnet")(digits_caps)
