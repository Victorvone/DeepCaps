    ...
    def __init__(self, model_list, voting="hard", weights=None, nb_classes=None):
        ...
        if weights is not None:
            if len(weights) != len(model_list):
                raise (
                    "Number of models {0} and length of weight vector {1} has to match.".format(
                        len(weights), len(model_list)
                    )
                )

    def predict(self, X, batch_size=128, verbose=0):
        predictions = list(
            map(lambda model: model.predict(X, batch_size, verbose)[0], self.model_list)
        )
        nb_preds = len(X)
...
def resize(data_set):
    ...
    for i in range(data_set.shape[0]):
        ...
        X_temp.append(np.asarray(img, dtype=np.float32) / 255.0)
...
for i in range(loop_size):
    ...
    _, eval_model1 = m1.DeepCapsNet(
        input_shape=x_test64.shape[1:], n_class=10, routings=3
    )
