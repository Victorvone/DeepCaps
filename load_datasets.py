...
def load_svhn():
    ...
    x_train = np.einsum("ijkl->lijk", svhn["X"]).astype(np.float32) / 255.0
    y_train = to_categorical((svhn["y"] - 1).astype("float32"))

    svhn_test = spio.loadmat("test_32x32.mat")
    x_test = np.einsum("ijkl->lijk", svhn_test["X"]).astype(np.float32) / 255.0
...
def get_annotations_map():
    valAnnotationsPath = (
        "./datasets/tiny_imagenet/tiny-imagenet-200/val/val_annotations.txt"
    )
...
def resize(data_set, size):
    ...
    X_temp = np.array(X_temp, dtype=np.float32) / 255.0
    return X_temp
