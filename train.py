...
def train(model, data, hard_training, args):
    ...
    checkpoint1 = CustomModelCheckpoint(
        model,
        args.save_dir + "/best_weights_1" + appendix + ".h5",
    ...
    checkpoint2 = CustomModelCheckpoint(
        model,
        args.save_dir + "/best_weights_2" + appendix + ".h5",
        monitor="val_capsnet_acc",
        save_best_only=True,
        save_weights_only=True,
        verbose=1,
...
model, eval_model = DeepCapsNet28(
    input_shape=x_train.shape[1:], n_class=y_train.shape[1], routings=args.routings
)  # for 28*28
